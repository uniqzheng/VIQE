function Q_final = calc_SLEEQ1_Score(test_video,width, height, framerate, pixfmt, sigma, blocksizerow, blocksizecol, blockrowoverlap, blockcoloverlap, sh_th)
    Q_final = [];
...
% 	for fr = 1:2:nb_frames-1
    for fr = floor(framerate/2):framerate:nb_frames-2    
        ...
        matrix_one = ones(block_rownum,block_colnum);
        Q_patch = (matrix_one-norm_averg_dif_patch).*spatial_delta + norm_averg_dif_patch.*temporal_delta;
        
        sharpness_delta = abs(org_patch_sharpness-org_blr_patch_sharpness);
        Q_patch = reshape(Q_patch,1,block_rownum*block_colnum);
    ...
    flat_sharpbox = reshape(sharpness_box,1,size(patch_box,1)*size(patch_box,2));
    sigTresh = prctile(flat_sharpbox(:),sh_th);
    IX = find(flat_sharpbox(:)>sigTresh);
    flat_patchbox = flat_patchbox(IX);
    Q_final = nanmean(flat_patchbox);
    fclose(test_file);
end
